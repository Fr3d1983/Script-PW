...
log_filename = f"log_generazione_utenti_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(filename=log_filename, level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# Logger per l'audit
audit_logger = logging.getLogger("AuditLogger")
audit_handler = logging.FileHandler(f"audit_generazione_utenti_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
